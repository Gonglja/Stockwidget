...
- **自选管理**：增/删/改/上移/下移 + 勾选显示（`checked_codes`）；代码规格化去重规则：
  ...
  - 非法输入回退到上次有效值
- **其他**：仅显示数字（`short_code`）、名称截断 0–4 字（`name_length`）；刷新间隔 1/2/3/5/10/15/30/60s；全局快捷键可改（默认 `Ctrl+Alt+F`）；开机启动；程序图标（默认/系统 6 种/自定义文件）；位置记忆；隐藏暂停刷新、显示恢复；错误提示（网络异常显示「无网络连接」，保留上次有效数据）。
