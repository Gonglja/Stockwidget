...
| 排序/别名入口不可发现 | 追加右下角配置按钮 + 「自定义配置」对话框（见 §10）；点击表头与行内右键保留为快捷方式 |

---

## 10. 追加需求（2026-09-17）：右下角配置入口

用户反馈「排序没做」——功能已实现且有测试覆盖，但**唯一入口是点击表头**，而 `header_visible` 默认 `false`，默认状态下没有任何可见入口。追加：

**10.1 浮窗右下角配置按钮**
- `FloatWindow::paintEvent` 画 12×12 空心圆环（不用字体符号，避免缺字形），位置：右下角内缩 4px；默认 alpha 120，悬停 alpha 255 且中心填充点。
- 布局：`m_vbox` 内边距由 `(10,6,10,6)` 改为 `(10,6,20,20)`（`kGearSize + 2*kGearMargin`），窗口宽 +10、高 +14；按钮只占内边距，不遮任何单元格。
- 交互：命中区域按下 → 不拖动、不单击隐藏；抬起仍在区内 → 打开「自定义配置」；区内双击也不隐藏。
- 新增配置键 `gear_visible`（bool，默认 `true`）+ 浮窗右键勾选项「显示配置按钮」；关闭后内边距回退为 `(10,6,10,6)`。
- 悬停依赖 `setMouseTracking(true)`（child 无按键时才会收到 `MouseMove`）。

**10.2「自定义配置」对话框**（`src/ui/CustomConfigDialog.{h,cpp}`，非模态单例）
- 「排序」组：`指标`（不排序 + 11 个指标）+ `方向`（降序/升序，无指标时禁用）；改动即写 `sort_key`/`sort_asc` → 浮窗与表头指示器立即同步。
- 「自定义名称」组：`QTableWidget`（`代码`只读 | `自定义名称`可编辑），行 = `codes` 全部代码；编辑即写 `name_map`，**留空 = 删除别名**；写回时与既有映射合并（不在列表中的条目保留）。
- 每次 `showEvent` 都从当前配置重建，避免与设置面板/浮窗右键不同步。
- 单例用 `QPointer` 持有，父窗口销毁后自动置空（避免悬垂）。

**10.3 保留不动**：点击表头排序、浮窗行内右键「自定义名称…」、设置面板「自选列表」双击编辑。

**10.4 测试**：`tests/test_ui.cpp` 增 `customConfigDialogSortAndAliasEdit`、`gearOpensCustomConfigWithoutHiding`、`gearHiddenFallsBackToHide`。
